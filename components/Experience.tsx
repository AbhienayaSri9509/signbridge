import React, { Suspense } from 'react';
import { Canvas } from '@react-three/fiber';
import { Environment, ContactShadows, Stars, OrbitControls, Html } from '@react-three/drei';
import Avatar from './Avatar';
import { AvatarState } from '../types';

// Augment JSX namespace to recognize Three.js elements
declare global {
  namespace JSX {
    interface IntrinsicElements {
      ambientLight: any;
      spotLight: any;
      pointLight: any;
      group: any;
    }
  }
}

interface ExperienceProps {
  avatarState: AvatarState;
}

const LoadingFallback = () => (
  <Html center>
    <div className="flex flex-col items-center gap-2 bg-black/60 px-4 py-2 rounded-xl backdrop-blur-md border border-white/10 text-white text-xs">
      <div className="w-4 h-4 border-2 border-indigo-400 border-t-transparent rounded-full animate-spin" />
      <span>Loading 3D Avatar...</span>
    </div>
  </Html>
);

const Experience: React.FC<ExperienceProps> = ({ avatarState }) => {
  return (
    <Canvas
      camera={{ position: [0, 1.4, 2.5], fov: 40 }}
      shadows
      className="w-full h-full"
    >
      {/* Lighting */}
      <ambientLight intensity={0.6} />
      <spotLight
        position={[10, 10, 10]}
        angle={0.15}
        penumbra={1}
        intensity={1.5}
        castShadow
      />
      <pointLight position={[-10, 0, -10]} intensity={1} color="#db2777" />

      {/* Camera Controls - Enable zoom, rotate, and pan */}
      <OrbitControls
        enableZoom={true}
        enableRotate={true}
        enablePan={true}
        minDistance={1}
        maxDistance={8}
        minPolarAngle={0}
        maxPolarAngle={Math.PI / 1.5}
        target={[0, 0.8, 0]}
      />

      {/* Environment */}
      <Suspense fallback={null}>
        <Environment preset="city" />
      </Suspense>
      <Stars radius={100} depth={50} count={5000} factor={4} saturation={0} fade speed={1} />

      {/* The Avatar - Wrapped in Suspense */}
      <Suspense fallback={<LoadingFallback />}>
        <group position={[0, -0.9, 0]} scale={1.2}>
          <Avatar state={avatarState} />
        </group>
      </Suspense>

      {/* Shadows */}
      <ContactShadows
        resolution={1024}
        scale={10}
        blur={2}
        opacity={0.5}
        far={10}
        color="#831843"
      />
    </Canvas>
  );
};

export default Experience;